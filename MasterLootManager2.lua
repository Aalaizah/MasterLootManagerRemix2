...
local function AssignLoot(pItemLink, pPlayerName)
	...
	for winningPlayerIndex = 1, 40 do
		if (GetMasterLootCandidate(itemSlot, winningPlayerIndex) == pPlayerName) then
...
function MasterLootRolls:UpdateRollList()
	...
	for i = 1, self.rollCount do
		...
		local playerNameLabel = getglobal(buttonName .. "_PlayerName")
		local r, g, b = MasterLootManager:GetClassColor(select(2,UnitClass(playerName)) or "PRIEST") --The default is primarily for debug purposes (fake rolls), but also serves as a failsafe in case we ever do fail to get a unit's class
...
function MasterLootManager:FillLootTable()
	...
	for lootIndex = 1, GetNumLootItems() do
		if (LootSlotHasItem(lootIndex) and GetLootSlotType(lootIndex)==1) then
...
function MasterLootManager:CreateBasicSelectionFrame()
	if (self.selectionFrame == nil) then
		self.selectionFrame = CreateFrame("Frame", nil, MasterLootManager.frame)
...
function MasterLootManager:DropClicked(owner, value)
    MasterLootManager:DebugPrint("DropClicked(): info.owner == " .. owner:GetName());
...
function MasterLootManager:AssignDEClicked(buttonFrame)
  local disenchanter = UIDropDownMenu_GetText(MasterLootManager.deDropdownFrame)
	local itemLink = MasterLootTable:GetItemLink(self.currentItemIndex)
	if AssignLoot(itemLink,disenchanter) then
...
function MasterLootManager:AssignBankClicked(buttonFrame)
  local banker = UIDropDownMenu_GetText(MasterLootManager.bankDropdownFrame)
	local itemLink = MasterLootTable:GetItemLink(self.currentItemIndex)
	if AssignLoot(itemLink,banker) then
